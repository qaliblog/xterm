...
public class TermuxShellEnvironment extends AndroidShellEnvironment {
    ...
    public HashMap<String, String> getEnvironment(@NonNull Context currentPackageContext, boolean isFailSafe) {
        ...
        // When rootfs is used: avoid dpkg-preconfigure/apt errors, set HOME/PWD to /root to reduce getcwd() calls
        TermuxAppSharedPreferences appPrefs = TermuxAppSharedPreferences.build(currentPackageContext);
        if (appPrefs != null && appPrefs.isRootfsInstalled()) {
            environment.put("DEBIAN_FRONTEND", "noninteractive");
            environment.put(ENV_HOME, "/root");
            environment.put("PWD", "/root");
            environment.put("OLDPWD", "/root");
        }

        // If failsafe is not enabled, then we keep default PATH and TMPDIR so that system binaries can be used
        if (!isFailSafe) {
            environment.put(ENV_TMPDIR, TermuxConstants.TERMUX_TMP_PREFIX_DIR_PATH);
            String pathPrefix = TermuxConstants.TERMUX_BIN_PREFIX_DIR_PATH;
            if (appPrefs != null && appPrefs.isRootfsInstalled()) {
                pathPrefix = "/.xterm-app-bin:" + pathPrefix;
            }
            if (TermuxBootstrap.isAppPackageVariantAPTAndroid5()) {
                // Termux in android 5/6 era shipped busybox binaries in applets directory
                environment.put(ENV_PATH, pathPrefix + ":" + TermuxConstants.TERMUX_BIN_PREFIX_DIR_PATH + "/applets");
                ...
                environment.put(ENV_PATH, pathPrefix);
